...
public class GActCardCreateOrder extends  Activity
{
    ...
    private List						m_oGoodsTasteList=new ArrayList();
    ...
    public void onCardCreateOrderConfirm(View v)
    {
        if(m_txtGroupRoomCaption.getText().equals("")){
            ...
            bundle.putStringArrayList("tasteList", (ArrayList<String>) m_oGoodsTasteList);
    ...
    private LinearLayout OnCreateCardOrderGoods(Context	oContext,Map map,int nLineNum)
    {
        ...
        if(map==null) {
            ...
            txtName.setGravity(Gravity.CENTER);
            txtName.setId(5000+nLineNum);
            txtName.setTag(map);
            txtName.setClickable(true);
            txtName.setOnClickListener(m_oGoodsTasteChange);
            txtName.setTextSize(m_nFontSize * MainActivity.m_nPixelsFromDP);
            oHead.addView(txtName);
        }
...
//                txtTaste.setOnClickListener(m_oGoodsTasteChange);
//                txtTaste.setTag(map);
...
//        }
                ...
                try
                {
                    ...
                    for(int i=0;i<xContent.length();i++)
                    {
                        ...
                        if(map.get("uID").toString().length()>0)
                        {
                            ...
                            map.put("tasteuids","");//口味id
                            map.put("usertaste","");//口味
    ...
    private void  getTasteList()
    {
        GSvrChannel svr= 	new GSvrChannel()
        {
            public void onNetFailure(int statusCode,String strInfo)
            {
                ...
                MainActivity.onUserMessageBox("读取卡套餐", "读取口味失败，请检查网络是否畅通或者联系管理员！");
            }
            public void onNetSuccess(int nCode,String strInfo,JSONObject oJsonData)
            {
                if(nCode < 0)
        ...
        try
        {
            ...
            svr.m_oCurrentActivity = this;
            svr.onPost("api/mobile/getTasteList.do", requestDatas);
        }
        catch (JSONException e)
        {
            e.printStackTrace();
        ...
        public void onClick(View v)
        {
            ...
            builder.setItems(strItems, new DialogInterface.OnClickListener()
            {
                ...
                public void onClick(DialogInterface dialog, int which)
                {
                    ...
                    if(tv == null)
                        return;
                    int nGoodsNum= Integer.parseInt(map.get("goodsNum").toString());
                    int nUsedNum=onGetGoodsNumUsed(map.get("uID").toString());
                    if(tv!=null)
    ...
    View.OnClickListener m_oGoodsTasteChange = new View.OnClickListener()
    {
        public void onClick(View v)
        {
            ...
            final String strItems[] = new String[m_oGoodsTasteList.size()];
            final String struidItems[] = new String[m_oGoodsTasteList.size()];
            for(int i=0;i<m_oGoodsTasteList.size();i++) {
                strItems[i] = (String) ((Map) m_oGoodsTasteList.get(i)).get("caption");
                struidItems[i] = (String) ((Map) m_oGoodsTasteList.get(i)).get("uID");
            }
            //=====================================================================
            AlertDialog.Builder builder = new AlertDialog.Builder(GActCardCreateOrder.this);
            builder.setIcon(R.drawable.ic_launcher);
            ...
            builder.setItems(strItems, new DialogInterface.OnClickListener()
            {
                ...
                public void onClick(DialogInterface dialog, int which)
                {
                    String strTasteUser =strItems[which];
                    String strTasteUid =struidItems[which];
                    txtGoodsTaste.setTextColor(Color.rgb(0,0xff, 0));
                    Map map = (Map)txtGoodsTaste.getTag();
                    if(map == null)
                        return;
                    map.put("tasteuids",strTasteUid);
                    map.put("usertaste",strTasteUser);
